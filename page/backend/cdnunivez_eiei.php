<div class="container-sm  <?php echo $bg?> border border-2   mt-5 border-2 shadow-sm p-4 mb-4 rounded" data-aos="fade-down">
    <center>
        ...
        <div class="mb-2 mt-4">
            ...
            <select class="form-select" id="pop_3" aria-label="Default select example">
                ...
                if ($data['recom_3'] != "0") {
                    ...
                    $data_pd = $get_pd->fetch(PDO::FETCH_ASSOC);
                ?>
                    <option value="<?php echo $data['recom_3'] ?>"><?php echo $data_pd['c_name']; ?></option>
                    <?php
                    $all_p = dd_q("SELECT * FROM category ORDER BY c_id DESC");
                    while ($row = $all_p->fetch(PDO::FETCH_ASSOC)) {
        ...
        <div class="mb-2 mt-4">
            ...
            <select class="form-select" id="pop_4" aria-label="Default select example">
                ...
                if ($data['recom_4'] != "0") {
                    ...
                    $data_pd = $get_pd->fetch(PDO::FETCH_ASSOC);
                ?>
                    <option value="<?php echo $data['recom_4'] ?>"><?php echo $data_pd['c_name']; ?></option>
                    <?php
                    $all_p = dd_q("SELECT * FROM category ORDER BY c_id DESC");
                    while ($row = $all_p->fetch(PDO::FETCH_ASSOC)) {
...
<script type="text/javascript">
    $("#btn_regis").click(function(e) {
        ...
        $.ajax({
            type: 'POST',
            url: 'services/backend/crecom_update.php',
            data: formData,
            contentType: false,
            processData: false,
        }).done(function(res) {
